window.PortfolioUtils = {
  ...
  safeUrl(value, fallback = '#') {
    ...
    try {
      ...
      // Invalid URLs are never rendered as navigable links.
    }
    return fallback;
  },
  sanitizeRichHtml(html) {
    if (!html) return '';
    const allowedTags = new Set(['A', 'B', 'BLOCKQUOTE', 'BR', 'CODE', 'EM', 'H2', 'H3', 'H4', 'I', 'LI', 'OL', 'P', 'PRE', 'STRONG', 'U', 'UL']);
    ...
    elements.forEach((element) => {
      ...
      if (element.tagName === 'A') {
        const href = this.safeUrl(rawHref, '');
        if (href) {
