...
try {
    ...
    $jsessionid = ""
    foreach ($c in $session.Cookies.GetCookies( (New-Object System.Uri("https://www.linkedin.com")) )) {
        if ($c.Name -eq "JSESSIONID") {
            $jsessionid = $c.Value.Trim('"')
            break
        }
    }
    
    if (-not $jsessionid) {
        Write-Output "Failed to get JSESSIONID."
        exit
    }
    
    Write-Output "Got JSESSIONID: $jsessionid"
    ...
    Write-Output "Step 2: Fetching recommendations..."
